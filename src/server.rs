...
pub fn start(addr: &str) -> io::Result<coroutine::JoinHandle<()>> {
    ...
    go!(
        ...
        move || {
            ...
            for stream in listener.incoming() {
                ...
                go!(
                    builder,
                    move || if let Err(e) = each_connection_loop(&mut stream) {
                        //error!("service err = {e:?}");
...
#[cfg(unix)]
fn each_connection_loop(stream: &mut TcpStream) -> io::Result<()> {
